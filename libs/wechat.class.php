...
class Wechat
{
	...
	const GROUP_MEMBER_DELETEGROUPS_URL='/groups/delete?';
	...
	public function __construct($options)
	{
		...
		$this->access_token_filePath =isset($options['access_token_filePath'])?$options['access_token_filePath']:'';
	}
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		...
		//如果设置了缓存路径就开启了缓存
		if(isset($this->access_token_filePath)&&($this->access_token_filePath!='')){
			$theFilePath=$this->access_token_filePath.''.$cachename;
			//把旧的无用缓存文件删除
			if(file_exists($theFilePath)){
				unlink($theFilePath);	
			}
			
			$cacheArr=array($expired,$value,time());
			$cacheTrr=implode('++', $cacheArr);
			
			file_put_contents($theFilePath, $cacheTrr);
	...
	protected function getCache($cachename){
		...
		if(isset($this->access_token_filePath)&&($this->access_token_filePath!='')){
			$theFilePath=$this->access_token_filePath.''.$cachename;
			if(file_exists($theFilePath)){
				$theCache = file_get_contents($theFilePath);
				if(!isset($theCache)){
					return false;
				}
				$cacheArr=explode("++", $theCache);
				$exprise = $cacheArr[0];
				$success_token = $cacheArr[1];
				$theTime=$cacheArr[2];
				if(!isset($theTime)){
					return false;
				}
				if( (time() - $theTime) >= $exprise ){
					//删除文件
					unlink($theFilePath);	
					return false;
				}else{
...
//		return urldecode($result);
	...
	public function deleteGroups($groupid){
		...
		$result = $this->http_post(self::API_URL_PREFIX.self::GROUP_MEMBER_DELETEGROUPS_URL.'access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
